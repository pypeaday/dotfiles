...
| `nexus-copilot` | Wrapper for `nexus-copilot:nexus` with the same read-only infra approvals as `infra-copilot`; also available as `Nexus` in `cc` |
...
- Infra and Nexus profiles auto-approve Athena and Glue `list-*`, `get-*`,
  `batch-get-*`, and `describe-*` commands through both AWS wrappers, plus Glue
  `search-tables` and `check-schema-version-validity`. This includes reading
  existing Athena query results, not starting queries. The production wrapper
  blocks query/job execution and other mutations.
- Infra and Nexus profiles auto-approve `cloudtrail lookup-events` through both
  AWS wrappers for event-history inspection. Other CloudTrail operations remain
  blocked by the production wrapper.
- Infra and Nexus profiles auto-approve DataSync `list-*` and `describe-*`
  commands through both AWS wrappers. The production wrapper rejects DataSync
  mutations, including starting or cancelling task executions.
...
The Azure DevOps and Nexus profiles enable the `azure-devops` MCP server for
the `cat-aa` organization and load only the `core` and `work-items` domains.
Nexus also keeps its local Semble MCP server enabled for code search.
